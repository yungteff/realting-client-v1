...
public final class Player extends Entity {
	...
	public static List mruNodes = new List(400);
	...
	public Model getRotatedModel() {
		...
		if (super.gfxId != -1 && super.currentAnim != -1) {
			...
			if(spotAnim.animation != null) {
				if (FrameReader.animationlist[Integer.parseInt(Integer.toHexString(spotAnim.animation.frameIDs[0])
						.substring(0, Integer.toHexString(spotAnim.animation.frameIDs[0]).length() - 4), 16)].length == 0){
					model_2 = null;
				}
			}
	 ...
	 */
		...
		} else if (currentFrame != -1 && nextFrame != -1 && Settings.get(Settings.Data.TWEENING)) {
		...
		model_2.method466();
		model_2.face_skin = null;
		model_2.vertex_skin = null;
		return model_2;
	}
	...
	public void updatePlayer(ByteBuffer stream) {
		...
		skulled = stream.readUnsignedShort() == 1;
		...
		for (int partId = 0; partId < 12; partId++) {
			...
			if (partId == 0 && equipment[0] == 65535) {
				desc = MobDefinition.get(stream.readUnsignedShort());
				break;
			}
			if(partId == 8)
		...
		playerRights = stream.readUnsignedShort();
		loyaltyTitle = getTitle(stream.readUnsignedShort());
