...
import org.necrotic.client.graphics.rsinterface.Settings.Data;
...
public class GameRenderer extends Applet implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener, WindowListener, MouseWheelListener {
	...
	private void interfaceScrollCheck(MouseWheelEvent e) {
		try {
			...
			int tabInterfaceID = Client.tabID == -1 ? 0 : Client.tabInterfaceIDs[Client.tabID];
			...
			if (InterfaceID[WINDOW] != -1) {
				...
				if (GameFrame.getScreenMode() != ScreenMode.FIXED) {
					InterfaceSetting[OFFX] = (Client.instance.getScreenWidth() - 765) / 2;
					InterfaceSetting[OFFY] = (Client.instance.getScreenHeight() - 503) / 2;
	...
	public final void keyPressed(KeyEvent keyevent) {
		...
		if (!Settings.get(Data.NEW_FUNCTION_KEYS)) {
