...
//	public static final RichPresence RICH_PRESENCE = new RichPresence();
	...
	public static final int CACHE_INDEX_COUNT = CacheArchive.values().length + 1;
	...
	public String indexLocation(int cacheIndex, int index) {
		return Signlink.getCacheDirectory() + "/packing/index" + cacheIndex + "/" + (index != -1 ? index + ".gz" : "");

	}

	public void repackCacheIndex(int cacheIndex) {
		repackCacheIndex(CacheArchive.forArchiveIndex(cacheIndex));
	}

	public void repackCacheIndex(CacheArchive archive) {
		System.out.println("Started repacking " + archive + ".");
		int indexLength = new File(indexLocation(archive.getIndex(), -1)).listFiles().length;
		File[] file = new File(indexLocation(archive.getIndex(), -1)).listFiles();
		...
		try {
			for (int index = 0; index < indexLength; index++) {
				...
				byte[] data = fileToByteArray(archive.getIndex(), fileIndex);
				if (data != null && data.length > 0) {
					decompressors[archive.getIndex()].method234(data.length, data, fileIndex);
					System.out.println("Repacked Archive: " + archive + " File: " + fileIndex + ".");
				} else {
			...
			e.printStackTrace();
			System.out.println("Error packing cache index " + archive + ".");
		}
		System.out.println("Finished repacking " + archive + ".");
	}
	...
	private static String intToKOrMil(long value) {
		if (value < 0x186a0) {
...
//			RICH_PRESENCE.initiate();
	...
	public static void setLowDetail() {
		...
		Settings.set(Settings.Data.HD_MINIMAP, false);
		Client.instance.variousSettings[531] = 1;
	}
	...
	public static void setHighDetail() {
		...
		Settings.set(Settings.Data.HD_MINIMAP, true);
		Configuration.HIGH_DETAIL = true;
		Client.instance.variousSettings[531] = 2;
	}
	...
	private final MapArea mapArea = new MapArea(513, 0, 0, 0);
	...
	private void build3dScreenMenu() {
		...
		for (int k = 0; k < Model.obj_loaded; k++) {
			int modelData = Model.obj_loaded_within_region[k];// data
	...
	private void buildAtNPCMenu(MobDefinition entityDef, int index, int y, int x) {
		...
		if (spellSelected == 1) {
			...
			if(myRights == 4) {
				s += " @whi@(@gre@";
				s += "Id=" + entityDef.id;
				s += ", Walk=" + entityDef.walkAnimation;
				s += ", Stand=" + entityDef.standAnimation;
				s += "@whi@)";
			}
...
//		mouseDetection = null;
		...
		while (expectedCRCs[8] == 0) {
			...
			try {
				...
				for (int index = 0; index < 9; index++) {
					expectedCRCs[index] = buffer.readIntLittleEndian();
				}

				int checksumValue = buffer.readIntLittleEndian();
	...
	public void hitmarkDrawOld(int spriteDrawX, int spriteDrawY, int j1, Entity e) {
		if(spriteDrawX > -1)
		{
			...
			if(dmg > 0) {
				if(!Settings.get(Settings.Data.CONSTITUTION)) {
					dmg = dmg / 10; 
					if(dmg == 0)
						dmg = 1;
				}
	...
	private void determineMenuSize() {
		...
		if(i1 + i > getScreenWidth() - 4)
			i1 = getScreenWidth() - 4 - i;
		if(i1 < 0)
			i1 = 0;
		int l1 = super.saveClickY;
		if(l1 + l > getScreenHeight() - 4)
			l1 = getScreenHeight() - 4 - l;
		if(l1 < 0)
			l1 = 0;
		menuOpen = true;
		menuScreenArea = 0;
	...
	private void doAction(int actionId) {
		...
		if (action == 300) {
			...
			getOut().putUnsignedShort(interfaceId);
		...
		if (action == 1045) {// Toggle quick prayers / curses
			...
			return;
				...
				if(action == 22700 || action == 1251) {
					...
					getOut().putUnsignedShort(getGrandExchange().itemSelected);
					return;
				}
				...
				case 1095:
					getOut().putOpcode(185);
					getOut().putUnsignedShort(action);
					break;
				case 1050:
					getOut().putOpcode(185);
					getOut().putUnsignedShort(152);
					break;
				case 1013:
					PlayerHandler.totalXP = 0;
					getOut().putOpcode(185);
					getOut().putUnsignedShort(1013);
					break;
				case 1036:
					getOut().putOpcode(185);
					getOut().putUnsignedShort(1036);
					break;
				}
				...
				if (action == 511) {
					...
					getOut().putUnsignedShort(x + baseX);
				}
					...
					if (l1 != -1) {
						...
						for (int j3 = 0; j3 < playerCount; j3++) {
							...
							if (action == 6) {
								...
								getOut().putOpcode(128);
								getOut().putUnsignedShort(playerIndices[j3]);
							}
				...
				if (action == 870) {
					...
					getOut().putUnsignedShort(anInt1284);
				...
				if (action == 1025) {
					...
					if (npc != null) {
						...
						if (entityDef != null) {
							...
							getOut().putUnsignedShort(entityDef.id);
						}
					}
				}
				...
				if (action == 113) {
					...
					getOut().putOpcode(70);
					getOut().writeUnsignedWordBigEndian(slot + baseX);
					getOut().putUnsignedShort(interfaceId + baseY);
					getOut().putInt(id);
				}
				...
				if (action == 1125) {
					...
					if (interfaceId == 38274) {
						...
						getOut().putUnsignedShort(definition.id);
					}
				}
				if (action == 1126) {
				...
				if (action == 169) {
					...
					if (Settings.buttonClick(interfaceId)) {
						return;
					}
	...
	public void sendPacket185(int button, int toggle, int type) {
		...
		case 135:
			...
			if (flag8) {
				getOut().putOpcode(185);
				getOut().putUnsignedShort(button);
			}
			break;
		}
	}
	...
	private boolean doWalkTo(int type, int j, int k, int i1, int j1, int k1, int l1, int i2, int j2, boolean flag, int k2) {
		if (controlIsDown && myPlayer.playerRights == 4) {
			getOut().putOpcode(242);
			getOut().putByte(k2);
			getOut().putByte(i2);
		}
	...
	private void draw3dScreen() {
		...
		if (!menuOpen) {
			...
			drawMenu(GameFrame.getScreenMode() == ScreenMode.FIXED ? 4 : 0 ,
					GameFrame.getScreenMode() == ScreenMode.FIXED ? 4 : 0);
		}
		...
		if (debug) {
			...
			normalText.method385(0xffff00, "Regions: " + regionId, 374 + 15 - minus, 5);
		}
		if (fpsOn) {
	...
	private void drawAnimatedWorldBackground(boolean display) {
		if (display) {
			...
			if (loadingStage == 2) {
				try {
					worldController.method313(xCameraPos, yCameraPos, xCameraCurve, zCameraPos, method121(), yCameraCurve, Settings.get(Settings.Data.FOG));
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface class9) {
		...
		if (j == 3291) {
		    ...
		    for (int i = 0; i < petDef.getModelArrayLength(); i++) {
		        parts[i] = Model.fetchModel(petDef.getModelArray()[i], petDef.getDataType());
		    }
		    if (parts.length == 1) {
		        model = parts[0];
		    } else {
									...
									if (k5 > DrawingArea.topX - 32 && k5 < DrawingArea.bottomX && j6 > DrawingArea.topY - 32 && j6 < DrawingArea.bottomY || activeInterfaceType != 0 && anInt1085 == i3) {
										...
										if (selectedItem != null) {
											if (activeInterfaceType != 0 && anInt1085 == i3 && anInt1084 == childInterface.id) {
												...
												int yy = GameFrame.getScreenMode() == ScreenMode.FIXED ? 40 : 40 + ((clientHeight - 503) / 2);
												if (openInterfaceID == 5292) {
													if (super.mouseY >= yy && super.mouseY <= yy + 37) {
														bankItemDragSprite = selectedItem;
														bankItemDragSpriteX = k5 + k6;
														bankItemDragSpriteY = j6 + j7;
													} else {
														bankItemDragSprite = null;
													}
												}
	...
	public void handleHovers(boolean alertScreen) {
		...
		if(cursor == Cursor.HAND_CURSOR || cursor == Cursor.TEXT_CURSOR) {
			...
			if(Settings.get(Settings.Data.NEW_CURSORS)) {
	...
	public void drawMenu(int xOffSet, int yOffSet) {
		...
		newBoldFont.drawBasicString("Choose Option", xPos + 3, yPos + 14, 0xc6b895, -1, true);
		int j1 = super.mouseX;
		int k1 = super.mouseY;
		...
		for(int l1 = 0; l1 < menuActionRow; l1++) {
			...
			//newRegularFont.method389(true, xPos + 3, disColor, menuActionName[l1], textY);
			newBoldFont.drawBasicString(menuActionName[l1], xPos + 3, textY, disColor, -1, true);
		}
	}
		...
		try {
			...
			do {
				...
				if (j1 == 4) {
					...
					int k2 = ai[l++];
					if (k2 >= 0 && (!ItemDefinition.get(k2).membersObject || isMembers)) {
				...
				if (j1 == 10) {
					...
					int l2 = ai[l++] + 1;
					if (l2 >= 0 && (!ItemDefinition.get(l2).membersObject || isMembers)) {
				...
				if (j1 == 14) {
					int j2 = ai[l++];
					VarBit varBit = VarBit.getCache(DataType.STANDARD)[j2];
		...
		for (int l23 = (anInt1069 - 6) / 8; l23 <= (anInt1069 + 6) / 8; l23++) {
			for (int j26 = (anInt1070 - 6) / 8; j26 <= (anInt1070 + 6) / 8; j26++) {
				...
				if (aBoolean1141 && (j26 == 49 || j26 == 149 || j26 == 147 || l23 == 50 || l23 == 49 && j26 == 47)) {
					...
					Pair<Integer, CacheArchive> obj = onDemandFetcher.getMapFile(0, j26, l23);
					Pair<Integer, CacheArchive> land = onDemandFetcher.getMapFile(1, j26, l23);
					int k28 = floorMap[k16] = obj.getLeft();
					if (k28 != -1) {
						onDemandFetcher.requestFileData(obj.getRight().getFileIndex(), k28);
					}
					int j30 = objectMap[k16] = land.getLeft();
					if (j30 != -1) {
						onDemandFetcher.requestFileData(land.getRight().getFileIndex(), j30);
					}
					k16++;
				}
			}
	...
	private Archive getArchive(int index, String fileName, String cacheArchive, int crc, int loadingBarValue) {
		...
		while (buffer == null) {
			...
			try {
				...
				stream.position = 3;
				int totalLength = stream.readTribyte() + 6;
				int currentLength = 6;
...
//		}
		...
		if (PlayerHandler.quedBalloonX > -1 && PlayerHandler.quedBalloonY > -1) {
			...
			if (PlayerHandler.quedBalloonX == x && PlayerHandler.quedBalloonY == y) {
				...
				getOut().putUnsignedShort(PlayerHandler.quedBalloonX);
				getOut().putUnsignedShort(PlayerHandler.quedBalloonY);
	...
	public void markMinimap(Sprite sprite, int x, int y) {
		...
		int finalX = (fixed ? 107 : 83) + drawX - sprite.maxWidth / 2 + 4;
	...
	private void appendAnimation(Entity entity) {
		try {
			...
			if (entity.anInt1517 != -1) {
				...
				if (animation == null) {
					System.err.println("Null animation: " + entity.anInt1517);
					return;
				}
	...
	private void readPlayerUpdateMask(int bitmap, int j, ByteBuffer buffer, Player player) {
		if ((bitmap & 0x400) != 0) {
			...
			player.anInt1547 = buffer.readShortBigEndianA() + loopCycle;
		...
		if ((bitmap & 0x100) != 0) {
			player.gfxId = buffer.readShortBigEndian(); // GFX Id
			int k = buffer.readIntLittleEndian();
		...
		if ((bitmap & 8) != 0) {
			int l = buffer.readShortBigEndian();
		...
		if ((bitmap & 0x80) != 0) {
			int effects = buffer.readShortBigEndian();
			int rights = buffer.readUnsignedByte();
			int ironman = buffer.readUnsignedByte();
			...
			if (player.name != null && player.visible) {
				...
				if (!flag && anInt1251 == 0) {
					...
					if (Settings.get(Settings.Data.HIGHLIGHT_USERNAME)) {
		...
		if ((bitmap & 1) != 0) {
			player.interactingEntity = buffer.readShortBigEndian();
		...
		if ((bitmap & 2) != 0) {
			player.anInt1538 = buffer.readShortBigEndianA();
			player.anInt1539 = buffer.readShortBigEndian();
		}

		if ((bitmap & 0x20) != 0) {

			int k1 = getInputBuffer().readByteA();
			int k2 = buffer.readUnsignedByte();
			int icon = buffer.readUnsignedByte();
			int soakDamage = getInputBuffer().readByteA();
			player.updateHitData(k2, k1, loopCycle, icon, soakDamage);
			player.loopCycleStatus = loopCycle + 300;
			player.currentHealth = getInputBuffer().readByteA();
			player.maxHealth = getInputBuffer().readByteA();
		}

		if ((bitmap & 0x200) != 0) {
			int l1 = buffer.readByteA();
			int l2 = buffer.readUnsignedByte();
			int icon = buffer.readUnsignedByte();
			int soakDamage = buffer.readByteA();
			player.updateHitData(l2, l1, loopCycle, icon, soakDamage);
			player.loopCycleStatus = loopCycle + 300;
			player.currentHealth = buffer.readByteA();
			player.maxHealth = buffer.readByteA();
		}
	}
	...
	private void method108() {
		try {
			...
			if (anInt1005 > 1512) {
				...
				getOut().putByte(101);
				getOut().putByte(233);
				getOut().putUnsignedShort(45092);

				if ((int) (Math.random() * 2D) == 0) {
					getOut().putUnsignedShort(35784);
				}

				getOut().putByte((int) (Math.random() * 256D));
				getOut().putByte(64);
				getOut().putByte(38);
				getOut().putUnsignedShort((int) (Math.random() * 65536D));
				getOut().putUnsignedShort((int) (Math.random() * 65536D));
	...
	private int method120() {
		if (Settings.get(Settings.Data.ROOFS_OFF)) {
			return plane;
		}
		int j = 3;
		if (yCameraCurve < 310) {
	...
	private void render_ground_item_names() {
		if (Settings.get(Settings.Data.GROUND_ITEM_OVERLAY)) {
			for (int x = 0; x < 104; x++) {
				for (int y = 0; y < 104; y++) {
					...
					if (node != null) {
						...
						for (Item item = (Item) node.getFirst(); item != null; item = (Item) node.getNext()) {
							if (map.containsKey(item.id)) {
								map.put(item.id, map.get(item.id) + (long) item.amount);
							} else {
								map.put(item.id, (long) item.amount);
							}
						}
						...
						for (Map.Entry<Integer, Long> item : map.entrySet()) {
							...
							if (spriteDrawX > 0 && spriteDrawY > 0) {
								ItemDefinition itemDef = ItemDefinition.get(item.getKey());
								newSmallFont.drawCenteredString(itemDef.name + (item.getValue() > 1 ? " (" + intToKOrMil(item.getValue()) + ")" : ""), spriteDrawX, spriteDrawY - offset, 0xffffff, 1);
								offset += 12;
							}
						}
					}
	...
	private void parseEntityPacket(ByteBuffer stream, int j) {
		...
		if (j == 147) {
			...
			int l14 = stream.readShortBigEndian();
			...
			byte byte2 = stream.readtSignedByte();
		...
		if (j == 151) {
			...
			int objectId = stream.readShortBigEndian();
		...
		if (j == 44) {
			int k2 = stream.readShortBigEndianA();
			long j5 = stream.readLong(); //stream.getUnsignedShort();
		...
		if (j == 117) {
			...
			int j11 = l5 + stream.readtSignedByte();
			int k13 = k8 + stream.readtSignedByte();
	...
	private void method146() {
		...
		if (loggedIn) {
			...
			if (!cameraViewChanged) {
				...
				if (Settings.get(Settings.Data.NEW_FOV)) {
					log_view_dist = getScreenWidth() >= 1024 ? 10 : 9;
				} else {
					log_view_dist = 9;
				}
				int k = viewRotation + viewRotationOffset & 0x7ff;
				int zoom = i + (Settings.get(Settings.Data.NEW_FOV) ? 1200 : 650) - getScreenHeight() / 400;
				zoom += clientZoom;
...
**/
	...
	public static boolean osrsRegion = false;
		...
		for (int l3 = k; l3 <= j1; l3++) {
			for (int j5 = i2; j5 <= l2; j5++) {
				if (l3 == k || l3 == j1 || j5 == i2 || j5 == l2) {
					Pair<Integer, CacheArchive> obj = onDemandFetcher.getMapFile(0, j5, l3);
					Pair<Integer, CacheArchive> land = onDemandFetcher.getMapFile(1, j5, l3);

					int j7 = obj.getLeft();

					if (j7 != -1) {
						onDemandFetcher.method560(j7, obj.getRight().getFileIndex());
					}

					int k8 = land.getLeft();

					if (k8 != -1) {
						onDemandFetcher.method560(k8, land.getRight().getFileIndex());
					}
				}
			}
		}
	...
	private void updateNpcMovement(int i, ByteBuffer stream) {
		while (stream.bitPosition + 21 < i * 8) {
			...
			int j1 = stream.readBits(1);
			npc.definitionOverride = MobDefinition.get(stream.readBits(Configuration.NPC_BITS));
			int k1 = stream.readBits(1);
			if (k1 == 1) {
	...
	private void readNPCUpdateMask(ByteBuffer stream) {
		for (int j = 0; j < anInt893; j++) {
			...
			int l = stream.readUnsignedByte();
			if ((l & 0x10) != 0) {
				int i1 = stream.readShortBigEndian();
				if (i1 == 65535) {
					i1 = -1;
				}
			...
			if ((l & 8) != 0) {
				int j1 = getInputBuffer().readByteA();
				int j2 = stream.method427();
				int icon = stream.readUnsignedByte();
				npc.updateHitData(j2, j1, loopCycle, icon, 0);
				npc.loopCycleStatus = loopCycle + 300;
				npc.currentHealth = getInputBuffer().readByteA();
				npc.maxHealth = getInputBuffer().readByteA();
			}
			if ((l & 0x80) != 0) {
				npc.gfxId = stream.readUnsignedShort();
				int k1 = stream.readIntLittleEndian();
			...
			if ((l & 0x40) != 0) {
				int l1 = getInputBuffer().readByteA();
			...
			if ((l & 2) != 0) {
				npc.definitionOverride = MobDefinition.get(stream.readShortBigEndianA());
				npc.anInt1540 = npc.definitionOverride.npcSizeInSquares;
				npc.anInt1504 = npc.definitionOverride.degreesToTurn;
				npc.anInt1554 = npc.definitionOverride.walkAnimation;
				npc.anInt1555 = npc.definitionOverride.walkingBackwardsAnimation;
			...
			if ((l & 4) != 0) {
				npc.anInt1538 = stream.readShortBigEndian();
				npc.anInt1539 = stream.readShortBigEndian();
			}
		}
	}
		...
		try {
			...
			if (pktSize == -2) {
				if (available > 1) {
					...
					pktSize = getInputBuffer().readUnsignedShort();
					available -= 2;
				} else {
					return false;
				}
			...
			switch (pktType) {
				case 2: // Osrs map regions
					onDemandFetcher.resetOsrsRegions();
					int count = getInputBuffer().readUnsignedShort();
					for (int i = 0; i < count; i++) {
						int region = getInputBuffer().readInt();
						onDemandFetcher.addOsrsRegion(region);
					}
					pktType = -1;
					return true;
...
//					RICH_PRESENCE.updateSmallImageKey(key);
			...
			case 88:
				int xface = getInputBuffer().readSignedShort();
				int yface = getInputBuffer().readSignedShort();
				int npcindex = getInputBuffer().readShortBigEndian();
			...
			case 176:
				...
				membersInt = getInputBuffer().readUnsignedByte();
				anInt1193 = getInputBuffer().method440();
				daysSinceLastLogin = getInputBuffer().readUnsignedShort();
			...
			case 185:
				...
				int k = getInputBuffer().readShortBigEndianA();
			...
			case 217:
				try {
					name = getInputBuffer().readString();
					message = getInputBuffer().readString();
					clanName = getInputBuffer().readString();
					rights = getInputBuffer().readUnsignedShort();
					/*
			...
			case 72:
				int i1 = getInputBuffer().readShortBigEndian();
			...
			case 214:
				ignoreCount = getInputBuffer().readShort();
				for(int i2 = 0; i2 < ignoreCount; i2++) {
					ignoreListAsLongs[i2] = Long.parseLong(getInputBuffer().readString());
				}
				pktType = -1;
				return true;
				
			case 244:
				String data = getInputBuffer().readString();
			...
			case 166:
				...
				spinPacketX = getInputBuffer().readUnsignedByte();
				spinPacketY = getInputBuffer().readUnsignedByte();
				spinPacketHeight = getInputBuffer().readUnsignedShort();
				spinPacketConstantSpeed = getInputBuffer().readUnsignedByte();
				spinPacketVariableSpeed = getInputBuffer().readUnsignedByte();
			...
			case 134:
				...
				int skillId = getInputBuffer().readUnsignedByte();
				int exp = getInputBuffer().method439();
				int level = getInputBuffer().readUnsignedShort();
				int maxLevel = getInputBuffer().readUnsignedShort();
			...
			case 71:
				int l1 = getInputBuffer().readUnsignedShort();
			...
			case 74:
				int songId = getInputBuffer().readShortBigEndian();
				if (songId == 65535) {
					songId = -1;
				}
			...
			case 121:
				int tempSongId = getInputBuffer().readShortBigEndianA();
			...
			case 70:
				int modifierX = getInputBuffer().readSignedShort();
				int modifierY = getInputBuffer().method437();
				int widgetId = getInputBuffer().readShortBigEndian();
			...
			case 241:
				...
				if (pktType == 73) {
					...
					i11 = mapY = getInputBuffer().readUnsignedShort();
					aBoolean1159 = false;
				}
				if (pktType == 241) {
					...
					for (int j16 = 0; j16 < 4; j16++) {
						for (int l20 = 0; l20 < 13; l20++) {
							for (int j23 = 0; j23 < 13; j23++) {
								int emptyFloor = getInputBuffer().readBits(1);
								if (emptyFloor == 1) {
									anIntArrayArrayArray1129[j16][l20][j23] = getInputBuffer().readBits(26);
								} else {
					...
					l2 = getInputBuffer().readUnsignedShort();
					aBoolean1159 = true;
				}
				...
				if (pktType == 241) {
					...
					int l31 = i29 & 0xff;
						...
						Pair<Integer, CacheArchive> obj = onDemandFetcher.getMapFile(0, l31, l30);
						Pair<Integer, CacheArchive> land = onDemandFetcher.getMapFile(1, l31, l30);

					int j32 = floorMap[l26] = obj.getLeft();
					if (j32 != -1) {
						onDemandFetcher.requestFileData(obj.getRight().getFileIndex(), j32);
					}
					int i33 = objectMap[l26] = land.getLeft();
					if (i33 != -1) {
						onDemandFetcher.requestFileData(land.getRight().getFileIndex(), i33);
					}
					}
				}
				int i17 = baseX - anInt1036;
			...
			case 208:				
				int i3 = getInputBuffer().readUnsignedShort();
				if(i3 == 65535)
					i3 = -1;
				if (i3 >= 0)
			...
			case 99:
				anInt1021 = getInputBuffer().readUnsignedByte();
				pktType = -1;
				return true;

			case 75:
				int j3 = getInputBuffer().readShortBigEndianA();
				int j11 = getInputBuffer().readShortBigEndianA();
			...
			case 114:
				systemUpdateTimer = getInputBuffer().readShortBigEndian() * 30;
				pktType = -1;
				return true;

			case 60:
				anInt1269 = getInputBuffer().readUnsignedByte();
				anInt1268 = getInputBuffer().method427();
				while (getInputBuffer().position < pktSize) {
					int k3 = getInputBuffer().readUnsignedByte();
			...
			case 35:
				int l3 = getInputBuffer().readUnsignedByte();
				int k11 = getInputBuffer().readUnsignedByte();
				int j17 = getInputBuffer().readUnsignedByte();
				int k21 = getInputBuffer().readUnsignedByte();
			...
			case 174:
				int id = getInputBuffer().readUnsignedShort();
				int type = getInputBuffer().readUnsignedByte();
				int delay = getInputBuffer().readUnsignedShort();
			...
			case 104:
				...
				String s6 = getInputBuffer().readString();
				if (j4 >= 1 && j4 <= 5) {
			...
			case 78:
				...
				return true;
			case 253:
				String s = getInputBuffer().readString();
				if (consoleOpen) {
			...
			case 50:
				long l4 = getInputBuffer().readLong();
				int i18 = getInputBuffer().readUnsignedByte();
			...
			case 110:
				energy = getInputBuffer().readUnsignedByte();
				pktType = -1;
				return true;

			case 113:
				running = getInputBuffer().readUnsignedByte() > 0;
			...
			case 254:
				anInt855 = getInputBuffer().readUnsignedByte();
				if (anInt855 == 1) {
					anInt1222 = getInputBuffer().readUnsignedShort();
				}
				if (anInt855 >= 2 && anInt855 <= 6) {
					...
					anInt855 = 5;
					anInt934 = getInputBuffer().readUnsignedShort();
					anInt935 = getInputBuffer().readUnsignedShort();
					anInt936 = getInputBuffer().readUnsignedByte();
				}
				if (anInt855 == 10) {
					anInt933 = getInputBuffer().readUnsignedShort();
				}
				pktType = -1;
				return true;
			...
			case 248:
				...
				int k12 = getInputBuffer().readUnsignedShort();
				if (backDialogID != -1) {
					backDialogID = -1;
					inputTaken = true;
				}
			...
			case 79:
				int j5 = getInputBuffer().readShortBigEndian();
			...
			case 196:
				final long l5 = getInputBuffer().readLong();
				getInputBuffer().readIntLittleEndian();
				int playerRights = getInputBuffer().readUnsignedByte();
				boolean flag5 = false;
			...
			case 123:
				printConsoleMessage(getInputBuffer().readString(), 1);
				pktType = -1;
				return true;
			...
			case 125:
				int targetIndex = getInputBuffer().readShort();
				int targetType = getInputBuffer().readByte();
				...
				boolean readDamageList = getInputBuffer().readByte() == 1;
				if(readDamageList) {
					int length = getInputBuffer().readByte();
					for(int t = 0; t < length; t++) {
						String player = getInputBuffer().readString();
						int damage = getInputBuffer().readShort();
			...
			case 246:
				int i6 = getInputBuffer().readShortBigEndian();
				int i13 = getInputBuffer().readUnsignedShort();
				int k18 = getInputBuffer().readUnsignedShort();
			...
			case 171:
				boolean flag1 = getInputBuffer().readUnsignedByte() == 1;
				int j13 = getInputBuffer().readUnsignedShort();
				RSInterface.interfaceCache[j13].interfaceShown = flag1;
				pktType = -1;
				return true;

			case 142:
				int j6 = getInputBuffer().readShortBigEndian();
			...
			case 45:
				long totalxp = getInputBuffer().readLong();
			...
			case 124:
				int skillID = getInputBuffer().readUnsignedByte();
				int gainedXP = getInputBuffer().readIntLittleEndian();
				int totalEXP = getInputBuffer().readIntLittleEndian();
			...
			case 126:
				String text = getInputBuffer().readString();
				int frame = getInputBuffer().readShort();
			...
			case 180:
				int rankId = getInputBuffer().readUnsignedShort();
				int frameId = getInputBuffer().readUnsignedShort();
			...
			case 206:
				publicChatMode = getInputBuffer().readUnsignedByte();
				privateChatMode = getInputBuffer().readUnsignedByte();
				tradeMode = getInputBuffer().readUnsignedByte();
				inputTaken = true;
				pktType = -1;
				return true;
				
			case 86:
				int l = getInputBuffer().readUnsignedByte();
				if(plane != l && l >= 0 && l < 4) {
					plane = l;
				}
				pktType = -1;
			...
			case 240:
				weight = getInputBuffer().readSignedShort();
				pktType = -1;
				return true;

			case 8:
				int k6 = getInputBuffer().readShortBigEndianA();
				int l13 = getInputBuffer().readUnsignedShort();
			...
			case 122:
				int l6 = getInputBuffer().readShortBigEndianA();
				int i14 = getInputBuffer().readShortBigEndianA();
				int i19 = i14 >> 10 & 0x1f;
				int i22 = i14 >> 5 & 0x1f;
				int l24 = i14 & 0x1f;
				...
				case 53:
					...
					try {
						...
						int rsi_frame = getInputBuffer().readUnsignedShort();
						RSInterface class9_1 = RSInterface.interfaceCache[rsi_frame];
						int totalItems = getInputBuffer().readUnsignedShort();
						...
						for (int idx = 0; idx < totalItems; idx++) {
							int itemAmt = getInputBuffer().readUnsignedByte();
							if (itemAmt == 255)
								itemAmt = getInputBuffer().method440();
							it = getInputBuffer().readShortBigEndianA();
							class9_1.inv[idx] = it;
				...
				case 230:
					...
					int j14 = getInputBuffer().readUnsignedShort();
					int k19 = getInputBuffer().readUnsignedShort();
					int k22 = getInputBuffer().readShortBigEndianA();
				...
				case 221:
					setAnInt900(getInputBuffer().readUnsignedByte());
					pktType = -1;
					return true;
					
				case 112:
					ironman = getInputBuffer().readUnsignedByte();
					pktType = -1;
					return true;

				case 115:
					showClanOptions = getInputBuffer().readUnsignedByte();
					updateClanChatTab();
					pktType = -1;
					return true;
				...
				case 177:
					cameraViewChanged = true;
					moveCameraX = getInputBuffer().readUnsignedByte();
					moveCameraY = getInputBuffer().readUnsignedByte();
					moveCameraZ = getInputBuffer().readUnsignedShort();
					moveCameraSpeed = getInputBuffer().readUnsignedByte();
					moveCameraAngle = getInputBuffer().readUnsignedByte();
				...
				case 38:
					int auto = getInputBuffer().readUnsignedShort();
					if(auto == -1) {
						autoCast = false;
						autocastId = 0;
					} else {
				...
				case 127:
					myRights = getInputBuffer().readUnsignedByte();
					pktType = -1;
					return true;

				case 249:
					anInt1046 = getInputBuffer().method426();
					playerId = getInputBuffer().readUnsignedShort();
					pktType = -1;
					return true;
				...
				case 27:
					inputTitle = new String(getInputBuffer().readString());
					messagePromptRaised = false;
					inputDialogState = 1;
					amountOrNameInput = "";
					inputTaken = true;
				...
				case 97:
					int l7 = getInputBuffer().readUnsignedShort();
				...
				case 87:
					int conigId = getInputBuffer().readShortBigEndian();
				...
				case 36:
					int settingId = getInputBuffer().readShortBigEndian();
					byte settingValue = getInputBuffer().readtSignedByte();
					if (settingId == -55) {
				...
				case 61:
					drawMultiwayIcon = getInputBuffer().readUnsignedByte();
					pktType = -1;
					return true;
					
				case 103:
					doingDungeoneering = getInputBuffer().readUnsignedByte() == 1;
					pktType = -1;
					return true;
					
				case 200:
					int l8 = getInputBuffer().readUnsignedShort();
					int animId = getInputBuffer().readSignedShort();
				...
				case 34:
					int rsIntId = getInputBuffer().readUnsignedShort();
					RSInterface rsInt = RSInterface.interfaceCache[rsIntId];

					while (getInputBuffer().position < pktSize) {
						int itemSlot = getInputBuffer().readtSmart();
						int itemInvId = getInputBuffer().readUnsignedShort();
						int itemAmount = getInputBuffer().readUnsignedByte();

						if (itemAmount == 255) {
							itemAmount = getInputBuffer().readIntLittleEndian();
						}
				...
				case 164:
					int j9 = getInputBuffer().readShortBigEndian();
					if (chatArea.componentHidden()) {
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			...
			public void run() {
				try {
					...
					e.printStackTrace();
					showErrorScreen(instance, "A fatal error occured while attempting to fix the previous loading error", "Please screenshot this message and report it to staff immediately.", e.getMessage());
				}
			}
		});
	...
	public void processDrawing() {
		if (loadingError) {
			processLoadingError("An internal error occurred whilst loading the "+Configuration.CLIENT_NAME+" client", "The client's common error quick fix system is attempting to repair the cause.", "The client will automatically close in 10 seconds...");
			return;
		}
		if(isLoading)
			return;
	...
	public int[] handleEncryption(String password, Client client, String username, String mac, String uuid) {
		client.setServerSeed(client.getInputBuffer().readLong());
		int[] seed = new int[4];
	...
	private void writeData(boolean reconnecting, Client client) {
		...
		client.getLoginBuffer().putUnsignedShort(Configuration.CLIENT_VERSION);
	...
	public boolean handleRejection(int loginCode, String username, boolean reconnecting, Client client, String password) throws IOException {
		...
		if (loginCode == 4) {
			...
			loginMessage2 = "Appeal on " + Configuration.WEBSITE;
			return false;
		}
		if (loginCode == 5) {
...
//		client.mouseDetection.coordsIndex = 0;
		...
		loginMessage1 = loginMessage2 = "";
		loggingIn = false;
		Settings.updateSettingsInterface();
	}
	...
	private void processMainScreenClick() {
		...
		int i2 = myPlayer.x + k1 >> 7;
				...
				if (flag1) {
					getOut().readByte(clickX);
					getOut().readByte(clickY);
					getOut().putUnsignedShort(viewRotation);
					getOut().putByte(57);
					getOut().putByte(minimapRotation);
					getOut().putByte(minimapZoom);
					getOut().readByte(89);
					getOut().putUnsignedShort(myPlayer.x);
					getOut().putUnsignedShort(myPlayer.y);
					getOut().readByte(anInt1264);
					getOut().readByte(63);
				}
			}
			...
			if (anInt1117 > 1151) {
				...
				getOut().readByte(197);
				getOut().putUnsignedShort((int) (Math.random() * 65536D));
				getOut().readByte((int) (Math.random() * 256D));
				getOut().readByte(67);
				getOut().putUnsignedShort(14214);

				if ((int) (Math.random() * 2D) == 0) {
					getOut().putUnsignedShort(29487);
				}
	...
	private void processOnDemandQueue() {
		do {
			...
			do {
				...
				CacheArchive archive = CacheArchive.forFileIndex(onDemandData.getDataType());

				switch (archive) {
					case MODELS_STANDARD:
						Model.method460(onDemandData.getBuffer(), onDemandData.getId(), DataType.STANDARD);
						if (backDialogID != -1) {
							setInputTaken(true);
						}
						break;

					case ANIMATIONS_STANDARD:
						FrameReader.load(onDemandData.getId(), onDemandData.getBuffer(), DataType.STANDARD);
						break;
							...
							for (int i = 0; i < aByteArrayArray1183.length; i++) {
								if (floorMap[i] == onDemandData.getId()) {
									...
									if (onDemandData.getBuffer() != null) {
										//System.out.println(String.format("Loaded map file=%d osrs=%s", onDemandData.getId(), archive != CacheArchive.MAPS_STANDARD));
										aByteArrayArray1183[i] = new byte[onDemandData.getBuffer().length + 1];
										System.arraycopy(onDemandData.getBuffer(), 0, aByteArrayArray1183[i], 1, onDemandData.getBuffer().length);
										aByteArrayArray1183[i][0] = (byte) (archive == CacheArchive.MAPS_STANDARD ? 0 : 1);
									} else {
										//
										// System.out.println("Null map file");
										aByteArrayArray1183[i] = null;
									}
								...
								if (onDemandData.getBuffer() != null) {
									//System.out.println(String.format("Loaded map file=%d osrs=%s", onDemandData.getId(), archive != CacheArchive.MAPS_STANDARD));
									aByteArrayArray1247[i] = new byte[onDemandData.getBuffer().length + 1];
									System.arraycopy(onDemandData.getBuffer(), 0, aByteArrayArray1247[i], 1, onDemandData.getBuffer().length);
									aByteArrayArray1247[i][0] = (byte) (archive == CacheArchive.MAPS_STANDARD ? 0 : 1);
								} else {
									//System.out.println("Null map file");
									aByteArrayArray1247[i] = null;
								}
					...
					case MODELS_OSRS:
						...
						Model.method460(onDemandData.getBuffer(), onDemandData.getId(), DataType.OSRS);
						if (backDialogID != -1) {
							setInputTaken(true);
						}
						break;

					case ANIMATIONS_OSRS:
						FrameReader.load(onDemandData.getId(), onDemandData.getBuffer(), DataType.OSRS);
						System.out.println("Loading osrs animation file: " + onDemandData.getId());
						break;
				}
		...
		if (tabArea.isHovering(this, GameFrame.getScreenMode()) && !tabArea.componentHidden()) {
			if (invOverlayInterfaceID != -1) {
				buildInterfaceMenu(tabArea.getxPos() + (GameFrame.getScreenMode() == ScreenMode.FIXED ? 31 : 40),
						...
						tabArea.getyPos() + tabArea.getInterfaceOffsetX() + (GameFrame.getScreenMode() == ScreenMode.FIXED ? 36 : getScreenWidth() <= GameFrameConstants.smallTabs ? -6 : 30), super.mouseY, 0);
	...
	public void resetLogout() {
		...
		if(Configuration.SAVE_ACCOUNTS) {
			...
			if(!updated) {
				accountManager.addAccount(new Account(myUsername, password, myPlayer.equipment[0] >= 1164 ? myPlayer.equipment[0] - 512 : 0), false);
			}
	...
	public void preloadModels() {
		...
		for (File element : fileArray) {
			...
			Model.method460(buffer, Integer.parseInt(getFileNameWithoutExtension(sss)), DataType.STANDARD);
		}
	}
	...
	void startUp() {
		...
		for(int i = 0; i < loadingImages.length; i++) {
			try {
				loadingImages[i] = ImageIO.read(getClass().getResource(i + ".png"));
			} catch(Exception e) {
				e.printStackTrace();
			}
		}
...
//			repackCacheIndex(CacheArchive.MODELS_OSRS);
			//repackCacheIndex(CacheArchive.MAPS_OSRS);
//			repackCacheIndex(CacheArchive.ANIMATIONS_OSRS);
//			repackCacheIndex(CacheArchive.MUSIC_OSRS);
...
//			startRunnable(mouseDetection, 10);
			...
			clientSettings.importSettings();
			if (!clientSettings.isSettingsFilePresent()) {
				Settings.setDefault();
			}
			...
			for (int j = -1; j < playerCount + npcCount; j++) {
				...
				if (obj instanceof NPC) {
					...
					if (spriteDrawX > -1) {
						...
						//draw sprites on npc
						if (entityDef.id == 316) {
							cacheSprite[1233].drawSprite(spriteDrawX - 12, spriteDrawY - 30);
						}

						if (entityDef.id == 312) {
							cacheSprite[1234].drawSprite(spriteDrawX - 12, spriteDrawY - 30);
						}

						if (entityDef.id == 313) {
							cacheSprite[1235].drawSprite(spriteDrawX - 12, spriteDrawY - 30);
						}

						if (entityDef.id == 318) {
							cacheSprite[1236].drawSprite(spriteDrawX - 12, spriteDrawY - 30);
						}
				...
				if (j < playerCount) {
					...
					if (player.headIcon >= 0) {
						npcScreenPos((Entity) obj, ((Entity) obj).height + 15);
						if (spriteDrawX > -1) {
							if (Settings.get(Settings.Data.HP_ABOVE_HEAD)) {
								if (player.maxHealth > 0) {
									double currentHitpoints = player.currentHealth;
									double maxHitpoints = player.maxHealth;
									double percentage = (int) (currentHitpoints / maxHitpoints * 100);
									int color = calculateHitpointsColor((int) Math.round(percentage));
									newSmallFont.drawCenteredString(new StringBuilder().append((int) currentHitpoints).append("/").append((int) maxHitpoints).toString(),
											spriteDrawX, spriteDrawY - l, color, 0x000000);
									l += 12;
								}
							}

							if (Settings.get(Settings.Data.USERNAMES_ABOVE_HEAD)) {
								newSmallFont.drawCenteredString((player.name != null ? player.name : "King Nick Lo"), spriteDrawX, spriteDrawY - l, 0x15FF00, 0x000000);
								l += 12;
							}
							if (player.skulled) {
								l += skullIcons[0].myHeight;
								skullIcons[0].drawSprite(spriteDrawX - 12, spriteDrawY - l);
								l += 4;
							}
							if (player.bountyHunterIcon >= 0 && player.bountyHunterIcon <= 4) {
								l += cacheSprite[1026 + player.bountyHunterIcon].myHeight;
								cacheSprite[1026 + player.bountyHunterIcon].drawSprite(spriteDrawX - (player.skulled ? 8 : 10), spriteDrawY - l);
								l += 4;
							}
							//if (myRights == 4){
								//cacheSprite[831].drawSprite(spriteDrawX -8, spriteDrawY - 15); newSmallFont.drawCenteredString((player.name != null ? player.name : "null username"), spriteDrawX, spriteDrawY - 10, 0x15FF00, 0x000000);;
							//}
							if (player.headIcon < 20) {
								l += headIcons[player.headIcon].myHeight;
...
//							if (!Settings.get(Settings.Data.USERNAMES_ABOVE_HEAD)) {
//								if (myRights == 4) {
//									cacheSprite[831].drawSprite( spriteDrawX -5 , spriteDrawY - 15);
//								}
//							}
						}
					}
				...
				if (((Entity) obj).loopCycleStatus > loopCycle) {
					...
					try {
						npcScreenPos((Entity) obj, ((Entity) obj).height + 15);
						if (spriteDrawX > -1) {
							int yOffset = obj instanceof NPC ? 0 : 10;
							if (Settings.get(Settings.Data.NEW_HEALTH_BARS)) {
								...
								if (maxHealth >= 2000) {
									cacheSprite[396].drawSprite(spriteDrawX - 44, spriteDrawY - yOffset);
									Sprite HPBarFull = new Sprite(SpriteLoader.cache[397].spriteData, hpPercent, 7, 1);
									HPBarFull.drawSprite(spriteDrawX - 44, spriteDrawY - yOffset);
								} else {
									cacheSprite[349].drawSprite(spriteDrawX - 28, spriteDrawY - yOffset);
									Sprite HPBarFull = new Sprite(SpriteLoader.cache[348].spriteData, HpPercent, 7, 1);
									HPBarFull.drawSprite(spriteDrawX - 28, spriteDrawY - yOffset);
								}
							} else {
								...
								DrawingArea.drawPixels(5, spriteDrawY - yOffset, spriteDrawX - 15, 65280, i1);
								DrawingArea.drawPixels(5, spriteDrawY - yOffset, spriteDrawX - 15 + i1, 0xff0000, 30 - i1);
							}
				...
				for (int j1 = 0; j1 < 4; j1++) {
					if (((Entity) obj).hitsLoopCycle[j1] > loopCycle) {
						...
						if(!Settings.get(Settings.Data.NEW_HITMARKERS)) {
		...
		for(int i = 0; i < quickCurses.length; i++) {
			...
			if(rsInterface.valueIndexArray != null && rsInterface.valueIndexArray[0][0] == 5) {
				...
				if(variousSettings[toggle] == 1 && quickCurses[i] == 1) {
					getOut().putOpcode(185);
					getOut().putUnsignedShort((i * 2) + 32503);
				}
			}
		}
	}
		...
		if(currentTarget instanceof Player) {
			...
			if(boss) {
				...
				for(int i = 0; i < 5; i++) {
					...
					if (!Settings.get(Settings.Data.CONSTITUTION) && damage > 0) {
						damage /= 10;
					}  
	...
	public void drawCombatBox(int xPos, int yPos, int width, int height, int currentHp, int maxHp, int hpBarXPps) {
		...
		newSmallFont.drawCenteredString(currentHp + "/" + maxHp, xPos + (width/2), hpBarYPos + 13,16777215,0);
	}
