...
public final class Background extends DrawingArea {
	...
	public Background(Archive streamLoader, String s, int i) {
		...
		palette = new int[j];
		for (int k = 0; k < j - 1; k++) {
			palette[k + 1] = stream_1.readTribyte();
		}
		...
		imgWidth = stream_1.readUnsignedShort();
		imgHeight = stream_1.readUnsignedShort();
		int i1 = stream_1.readUnsignedByte();
		int j1 = imgWidth * imgHeight;
		imgPixels = new byte[j1];
		if (i1 == 0) {
			for (int k1 = 0; k1 < j1; k1++) {
				imgPixels[k1] = stream.readtSignedByte();
			}
		...
		if (i1 == 1) {
			for (int l1 = 0; l1 < imgWidth; l1++) {
				for (int i2 = 0; i2 < imgHeight; i2++) {
					imgPixels[l1 + i2 * imgWidth] = stream.readtSignedByte();
				}
