...
public class CustomInterfaces extends RSInterface {
	...
	//	Client.getClient().getAccountManager().updateInterface();
		...
		modernMagicks();
		ancientMagicks();
		newJewelryInterface(tda);
		changeEnchantString();
	}

	public static void changeEnchantString() {
		interfaceCache[6005].message = "Level 87: Level-6+ Enchant";
		interfaceCache[6006].message = "For use on Onyx/Zenyte jewellery";
	}

	public static void newJewelryInterface(TextDrawingArea[] tda) {
		// Expanding old interface
		int[] containers = {4239, 4233, 4245, 18796};
		for (int container : containers) {
			RSInterface containerInterface = interfaceCache[container];
			containerInterface.width = 8;
			containerInterface.inv = new int[8];
			containerInterface.invStackSizes = new int[8];
			containerInterface.invSpritePadX = 0;
			containerInterface.actions = new String[5];
			containerInterface.actions[0] = "Make";
			containerInterface.actions[1] = "Make 5";
			containerInterface.actions[2] = "Make 10";
			containerInterface.actions[3] = "Make All";
		}

		// New interface
		final int borrowFromInterfaceId = 4161;
		int[] borrowed = {4228, 4231, 4232, 4234, 4237, 4238, 4240, 4243, 4244, 18789, 18795, 18803};

		final int interfaceId = 18875;
		int id = interfaceId;
		RSInterface inter = addInterface(id++);
		setChildren(4 + borrowed.length, inter);

		int y = 2;
		inter.child(0, id, 64 + 31, 18 + y);
		addSprite(id++, 1238); // Background

		inter.child(1, id, 238, 28 + y);
		addText(id++, "What would you like to make?", tda, 2, 15439903, true, true);

		inter.child(2, 33002, 387, 25 + y);
		inter.child(3, 33003, 387, 25 + y);

		for (int index = 0; index < borrowed.length; index++) {
			int[] coordinates = interfaceCache[borrowFromInterfaceId].getChildCoordinates(borrowed[index]);
			inter.child(4 + index, borrowed[index], coordinates[0] + 7, coordinates[1] + 4);
		}
	}

	public static void achievementsInfo(TextDrawingArea[] tda) {
		RSInterface tab = addTabInterface(24000);
		addSprite(24001, 1237);
		addText(24002, "Achievement name:", tda, 2, 15439903, false, true);
		addText(24003, "Difficulty:", tda, 2, 15439903, false, true);
		addText(24004, "Kill Shit", tda, 1, 16777215, false, true);
		addText(24005, "Grandmaster", tda, 1, 16777215, false, true);
		addText(24006, "Progress:", tda, 2, 15439903, false, true);
		addText(24007, "56/100", tda, 1, 16777215, false, true);
		addText(24008, "Achievement Diary", tda, 2, 15439903, true, true);
		addText(24009, "Points: 0", tda, 2, 15439903, true, true);
		tab.totalChildren(11);
		tab.child(0, 24001, 0, 0);
		tab.child(1, 24002, 169, 24);
		tab.child(2, 24003, 169, 46);
		tab.child(3, 24004, 298, 24);
		tab.child(4, 24005, 240, 47);
		tab.child(5, 24006, 169, 66);
		tab.child(6, 24007, 237, 66);
		tab.child(7, 24008, 260, 2);
		tab.child(8, 24009, 40, 2);
		tab.child(9, 24100, 169, 104);
		tab.child(10, 58400, 10, 24);
		RSInterface list = addTabInterface(24100);
		addText(24010, "Description", tda, 2, 15439903, true, true);
		addText(24011, "The Lore is in trouble. The evil wizard Sirium is at it again. \\n Are you able to help uncover the mystery of the Lore \\nand the power it withholds? ", tda, 1, 16777215, true, true);
		addText(24012, "Requirements", tda, 2, 15439903, true, true);
		addText(24013, "The ability to defeat a level 63 wizard. \\n A Firemaking level of 25. \\n A Crafting level of 35.\\nA fishing level of 70.\\nA Fletching level of 65.\\nMinimum of 120 combat level.", tda, 1, 16777215, true, true);
		addText(24014, "Rewards", tda, 2, 15439903, true, true);
		addText(24015, "Ability to access the lore anytime.\\nA skill experience lamp usable in any skill.\\n5,000x Coins\\n2 Quest Points", tda, 1, 16777215, true, true);

		list.totalChildren(6);
		list.child(0, 24010, 159, 2);
		list.child(1, 24011, 159, 22);
		list.child(2, 24012, 159, 72);
		list.child(3, 24013, 159, 92);
		list.child(4, 24014, 159, 182);
		list.child(5, 24015, 159, 202);
		list.height = 224;
		list.width = 320;
		list.scrollMax = 310;

		RSInterface scroll = addTabInterface(58400);
		scroll.totalChildren(171);
		int count = 0;
		String[] difficulty = { "@or1@Easy", "@or1@Intermediate", "@or1@Hard", "@or1@Master", "@or1@Grandmaster" };
		for (int i = 58401; i <= 58571; i++) {
			if ((i - 58401) % 25 == 0) {
				if (count == 5) {
					addClickableText(i, "", "View Achievement", tda, 1, 16744448, 100, 20);
				} else {
					addText(i, difficulty[count], tda, 2, 15439903, false, true);
					count++;
				}
			} else {
				addClickableText(i, "", "View Achievement", tda, 1, 16744448, 100, 20);
		...
		int id = 58401; for (int i = 0; (id <= 58571) && (i <= 170); i++) {
			scroll.children[i] = id;
			scroll.childX[i] = 5;
			int id2 = 58401; for (int i2 = 1; (id2 <= 58571) && (i2 <= 170); i2++) {
				scroll.childY[0] = 2;
				scroll.childY[i2] = (scroll.childY[(i2 - 1)] + 17);id2++;
			}
			id++;
		}
	...
	public void OsDropViewer() {
		...
		interfaceCache[33002].atActionType = 3;
		interfaceCache[33003].atActionType = 3;
	...
	private void playerPanel() {
		...
		addText(39155, Configuration.WEBSITE, tda, 2, 16750623, false, true);
	...
	private void playersOnline() {
		...
		addText(57007, Configuration.CLIENT_NAME, 0xff9933, true, true, -1, tda, 0);
    ...
    private void teleportGUI() {
		...
		//text
		addText(49005, Configuration.CLIENT_NAME + " Teleports", tda, 2, 0xFFFFFF);
	...
	private void equipmentScreenInterface() {
		...
		addText(18890, "Summoning: +0", tda, 1, 0xFF8900, false, true);// 19148
		addText(18891, "Absorb Melee: +0%", tda, 1, 0xFF9200, false, true);// 19149
		addText(18892, "Absorb Magic: +0%", tda, 1, 0xFF9200, false, true);// 19150
		addText(18893, "Absorb Ranged: +0%", tda, 1, 0xFF9200, false, true);// 19151
		addText(18894, "Ranged Strength: +0", tda, 1, 0xFF9200, false, true);// 19152
		addText(18895, "Magic Damage: +0%", tda, 1, 0xFF9200, false, true);// 19153
		...
		tab.child(44, 18890, 29, 211 - edit - 3);
		tab.child(45, 18891, 29, 225 - edit - 3);
		tab.child(46, 18892, 29, 239 - edit - 3);
		tab.child(47, 18893, 29, 253 - edit - 3);
		/* bottom */
		int edit2 = 33 - rofl, edit3 = 2;
		tab.child(19, 15114, 24, 223 + edit2);
		tab.child(14, 1686, 29, 262 - 24 + edit2 - edit3);
		tab.child(17, 18894, 29, 276 - 24 + edit2 - edit3);
		tab.child(48, 1687, 29, 290 - 24 + edit2 - edit3);
		tab.child(49, 18895, 29, 304 - 24 + edit2 - edit3);
		tab.child(15, 15125, 190, 200);
	...
	private void shopInterface() {
		...
		for (int child = 0; child < rsinterface.children.length; child++) {
			if (rsinterface.children[child] == 3900) {
				rsinterface.childX[child] = 41;
			}
		}
	...
	private void ancientMagicks() {
		RSInterface tab = addTabInterface(12855);
		int[] itfChildren = {
				1195, 12939, 12987, 13035, 12901, 12861, 13045, 12963, 13011, 13053, 12919, 12881, 13061,
	...
	private void modernMagicks() {
		...
		addButton(1195, 203, "Cast @gre@Home Teleport");
		RSInterface homeButton = interfaceCache[1195];
		homeButton.hoverType = 1196;
	...
	private void teleportsInterface() {
		final int[] teleportsCount = {
				13, 	// cities
				13, 	// monsters
				8,  	// dungeons
				19,  	// bosses
		...
		pageList.totalChildren(teleportsCount.length * 3);
		for (int i = 0; i < teleportsCount.length; i++) {
			addHoverButton(44008 + i, 1201, 123, 25, "Filter", 0, 44028 + i, 1);
			addHoveredButton(44028 + i, 1202, 123, 25, 44084);
			addText(44048 + i, "Teleport Name", tda, 2, 0xff9040, false, true);

			pageList.child(i, 44008 + i, 0, i * 26);
			pageList.child(i + (teleportsCount.length), 44028 + i, 0, i * 26);
			pageList.child(i + (teleportsCount.length * 2), 44048 + i, 6, 6 + i * 26);
		}

		pageList.width = 107;
		pageList.height = 260;
		pageList.scrollMax = (teleportsCount.length * 26) <= 260 ? 270 : (teleportsCount.length * 26);
		...
		for (int i = 0; i < teleportsCount.length; i++) {
			...
			page.totalChildren(3 * teleportsCount[i]);
			page.child(0, 44101 + b, 158, 17);
			int yy = 0;
			for (int j = 0; j < teleportsCount[i]; j++) {
				addHoverButton(44102 + j + b, 1205, 316, 25, "Select", 0, 44152 + j + b, 1);
				addHoveredButton(44152 + j + b, 1206, 316, 25, 44099);
				addText(44202 + j + b, "Teleport Name", tda, 1, 0xff9040, false, true);
				page.child(j, 44102 + j + b, 0, yy);
				page.child(j + (teleportsCount[i]), 44152 + j + b, 0, yy);
				page.child(j + (teleportsCount[i] * 2), 44202 + j + b, 5, 5 + yy);
				yy += 26;
			}
			page.width = 300;
			page.height = 180;
		...
		addText(44086, "asdf2", tda, 2, 0xff9040, true, true);
		addText(44087, "asdf1", tda, 0, 0xff9040, true, true);
		addText(44088, "asdf3", tda, 0, 0xff9040, true, true);
		...
		addText(44089, "", tda, 2, 0xff9040, true, true);
	...
	public void constructLunar() {
		...
		setBounds(1195, 11, 10, 0, Interface);
