...
public class TabArea extends GameFrame {
	...
	private void drawRedStones(Client client, ScreenMode screenMode) {
		if (isVisible()) {
			...
			if (client.invOverlayInterfaceID == -1 && Client.tabInterfaceIDs[Client.tabID] != -1) {
				if (GameFrameConstants.isOldGameframe()) {
					for (int i = 0; i < 14; i++) {
						int tabID = i;
						if(i == 7) {
							tabID = 13;
						...
						if (Client.tabID == tabID) {
							...
							if (getScreenMode() == ScreenMode.FIXED && i > 6) {
								y += 2;
							}

							Client.cacheSprite[screenMode == ScreenMode.FIXED ? i == 0 ? 8 : i == 6 ? 9 : i == 7 ? 10 : i == 13 ? 11 : 12 : 12].drawSprite(x +
									(getOffSetX() + 6 + (i > 6 ? i - 7 : Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED ? i : screenMode == ScreenMode.FIXED
											? i : i - 7) * Client.cacheSprite[screenMode == ScreenMode.FIXED ? i == 1 || i == 8 ? 8 : 12 : 12].myWidth + (screenMode != ScreenMode.FIXED
											|| i == 1 || i == 8 || i == 0 || i == 7 ? 0 : 5)), y + (i > 6 || screenMode != ScreenMode.FIXED ? getOffSetY() + getHeight() - 2
											- Client.cacheSprite[12].myHeight * (i < 7 && Client.clientWidth <= GameFrameConstants.smallTabs ? 2 : 1) - 1 : getOffSetY()));
						}
					}
				} else if (!GameFrameConstants.isOldGameframe()) {
					for (int i = 0; i < Client.tabInterfaceIDs.length; i++) {
						int offsetX = getOffSetX() + (i >= 8 ? i - 8 : i) * 30;
						int offsetY = getOffSetY();
						int resizeableOffset = 0;
						if (i >= 8) {
							// Bottom Tabs
							if (Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED) {
								// Resizeable small tabs
								offsetY = offsetY + 296;
								offsetX = offsetX - 8;
						...
						if (i < 8) {
							// Top tabs
							if (Client.clientWidth > GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED) {
								offsetX = offsetX - Client.cacheSprite[527].myWidth / 2 - 9;
								offsetY = offsetY + 297;
							} else if (screenMode != ScreenMode.FIXED) {
								offsetY = offsetY + 261;
								offsetX = offsetX - 9;
						...
						if (Client.tabID == tabID) {
							...
							if (Client.tabInterfaceIDs[tabID] != -1 && client.mouseX >= x + (screenMode == ScreenMode.FIXED ? 6 : 5) && client.mouseX <= x + 30 + 4 && client.mouseY >= y && client.mouseY <= y + 34) {
								Client.cacheSprite[524].drawAdvancedSprite(offsetX + (screenMode == ScreenMode.FIXED ? 6 : 5), offsetY);
							}
						}
					}
				}
	...
	int[][] gameFrameData = new int[][] {
			{ 528, 1219, 11, 8 }, // attack
			{ 529, 1220, 11, 8 }, // skills tab
			{ 530, 1221, 10, 8 }, // quest tab
			{ 782, 782, 10, 8 }, // achievements tab
			{ 532, 1222, 10, 8 }, // inventory tab
			{ 533, 1223, 12, 8 }, // equipment tab
			{ 534, 1224, 10, 8 }, // prayer tab
			{ 535, 1225, 10, 8 }, // magic tab
			{ -1, 781, 11, 8 }, // summoning tab
			{ 536, 1227, 11, 8 }, // friends tab
			{ 537, 1228, 11, 8 }, // ignores tab
			{ 541, 1226, 10, 10 }, // clan chat tab
			{ 538, 1230, 10, 8 }, // settings tab
			{ 539, 1231, 10, 8 }, // emotes tab
			{ 781, 782, 10, 7 }, // summon tab
			{ -1, -1, 11, 8 }, // extra tab
	};
		...
		if (isVisible()) {
			if (GameFrameConstants.isOldGameframe()) {
				...
				for (int i = 0; i < gameFrameData.length; i++) {
					if (client.invOverlayInterfaceID == -1) {
						int offsetX = getOffSetX() + gameFrameData[i][2] + (i >= 8 ? i - 8 : i) * 30;
						int offsetY = getOffSetY() + gameFrameData[i][3];
						if (i >= 8) {
							if (Client.clientWidth <= GameFrameConstants.smallTabs && screenMode != ScreenMode.FIXED) {
								offsetY = offsetY + 296;
								offsetX = offsetX - 9;
						...
						int spriteIndex = gameFrameData[i][GameFrameConstants.isOldGameframe() ? 1 : 0];
						if (spriteIndex != -1) {
	...
	protected void render(Client client, ScreenMode screenMode) {
		if (isVisible()) {
			...
			if (!componentHidden() && screenMode != ScreenMode.FIXED) {
				Client.cacheSprite[18].drawTransparentSprite(getOffSetX() + 39, getOffSetY() + getInterfaceOffsetX() + (Client.clientWidth <= GameFrameConstants.smallTabs ? -7 : 29), 100);
				Client.cacheSprite[7].drawSprite(getOffSetX() + 33, getOffSetY() + getInterfaceOffsetX() - (Client.clientWidth <= GameFrameConstants.smallTabs ? 13 : -23));
			...
			if (!componentHidden() || screenMode == ScreenMode.FIXED) {
				if (client.invOverlayInterfaceID != -1) {
					client.drawInterface(0, getOffSetX() + (screenMode == ScreenMode.FIXED ? 31 : 40),
							...
							getOffSetY() + getInterfaceOffsetX() + (screenMode == ScreenMode.FIXED ? 36 : Client.clientWidth <= GameFrameConstants.smallTabs ? -6 : 30));
					...
					client.drawInterface(0,
							getOffSetX() + (screenMode == ScreenMode.FIXED ? 31 : 40), tab,
							getOffSetY() + getInterfaceOffsetX() + (screenMode == ScreenMode.FIXED ? 36 : Client.clientWidth <= GameFrameConstants.smallTabs ? -6 : 30));

				}
				if(screenMode == ScreenMode.FIXED && client.menuOpen) {
					client.drawMenu(516, 168);
				}
			}
			if (screenMode == ScreenMode.FIXED) {
				if (client.tabAreaIP != null) {
	...
	public int getInterfaceOffsetX() {
		return getScreenMode() != ScreenMode.FIXED && GameFrameConstants.isOldGameframe() ? -3 : 0;
	}
	...
	private Sprite getSideIconForOldFrame(int tab) {
		...
		int spriteIndex = gameFrameData[tab][1];
		if(tab == 2) {
			if(Client.getClient().doingDungeoneering) {
				spriteIndex = 1031;
			}
		...
		return spriteIndex == -1 ? null : Client.cacheSprite[spriteIndex];
	}
