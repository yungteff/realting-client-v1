...
public final class IdentityKit {
	...
	public static void unpackConfig(Archive streamLoader) {
		...
		setLength(stream.readUnsignedShort());
	...
	public boolean method537() {
		...
		for (int j = 0; j < anIntArray658.length; j++) {
			if (!Model.method463(anIntArray658[j], DataType.STANDARD)) {
				flag = false;
			}
		}
	...
	public Model method538() {
		...
		for (int i = 0; i < anIntArray658.length; i++) {
			aclass30_sub2_sub4_sub6s[i] = Model.fetchModel(anIntArray658[i], DataType.STANDARD);
		}
	...
	public boolean method539() {
		...
		for (int i = 0; i < 5; i++) {
			if (anIntArray661[i] != -1 && !Model.method463(anIntArray661[i], DataType.STANDARD)) {
				flag1 = false;
			}
		}
	...
	public Model method540() {
		...
		for (int k = 0; k < 5; k++) {
			if (anIntArray661[k] != -1) {
				aclass30_sub2_sub4_sub6s[j++] = Model.fetchModel(anIntArray661[k], DataType.STANDARD);
			}
		}
	...
	private void readValues(ByteBuffer buffer) {
		do {
			...
			if (opcode == 1) {
				setAnInt657(buffer.readUnsignedByte());
			} else if (opcode == 2) {
				int j = buffer.readUnsignedByte();
				anIntArray658 = new int[j];
				for (int k = 0; k < j; k++) {
					anIntArray658[k] = buffer.readUnsignedShort();
				}

			} else if (opcode == 3) {
				setaBoolean662(true);
			} else if (opcode >= 40 && opcode < 50) {
				anIntArray659[opcode - 40] = buffer.readUnsignedShort();
			} else if (opcode >= 50 && opcode < 60) {
				anIntArray660[opcode - 50] = buffer.readUnsignedShort();
			} else if (opcode >= 60 && opcode < 70) {
				anIntArray661[opcode - 60] = buffer.readUnsignedShort();
			} else {
