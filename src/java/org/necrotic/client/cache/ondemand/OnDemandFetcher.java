...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
	...
	private HashSet<Integer> osrsRegions = new HashSet<>();
	...
	public OnDemandFetcher() {
		...
		fileStatus = new byte[CacheArchive.values().length][];
...
//			}
	...
	@Override
	public void get(int i, DataType dataType) {
		if (dataType == null) {
			throw new NullPointerException("Null dataType");
		}

		requestFileData(dataType == DataType.STANDARD ? CacheArchive.MODELS_STANDARD.getFileIndex() : CacheArchive.MODELS_OSRS.getFileIndex(), i);
	}
	...
	public Pair<Integer, CacheArchive> getMapFile(int landscapeOrObject, int regionY, int regionX)
	{
		int regionId = (regionX << 8) + regionY;
		boolean osrs = osrsRegions.contains(regionId);
		CacheArchive archive = osrs ? CacheArchive.MAPS_OSRS : CacheArchive.MAPS_STANDARD;
		int index = osrs ? 1 : 0;

		if (!ArrayUtils.contains(Client.regionId.split(", "), String.valueOf(regionId))) {
			Client.regionId += regionId + ", ";
		}

		for(int j1 = 0; j1 < mapIndices1[index].length; j1++) {
			if (mapIndices1[index][j1] == regionId) {
				if (landscapeOrObject == 0) {
					if (!osrs) {
						//Soulwars
						if (mapIndices2[0][j1] >= 3700 && mapIndices2[0][j1] <= 3840) {
							return Pair.of(mapIndices2[0][j1], archive);
						}
						for (int cheapHax : cheapHaxValues) {
							if (mapIndices2[0][j1] == cheapHax) {
								return Pair.of(mapIndices2[0][j1], archive);
							}
						}
					}
					//System.out.println("Object file=" + mapIndices2[index][j1] + ", region=" + regionId + ", osrs=" + osrs);
					return Pair.of(mapIndices2[index][j1], archive);
				} else {
					if (!osrs) {
						if (mapIndices3[0][j1] >= 3700 && mapIndices3[0][j1] <= 3840) {
							return Pair.of(mapIndices3[0][j1], archive);
						}
						for (int cheapHax : cheapHaxValues) {
							if (mapIndices3[0][j1] == cheapHax) {
								return Pair.of(mapIndices3[0][j1], archive);
							}
						}
					}
                   // System.out.println("Landscape file=" + mapIndices3[index][j1] + ", region=" + regionId + ", osrs=" + osrs);
					return Pair.of(mapIndices3[index][j1], archive);
				}
			}
		}

		//System.out.println("No map file found, region=" + regionId + ", osrs=" + osrs);
		return Pair.of(-1, archive);
	}
...
//			}
	...
	public void missing(OnDemandRequest request) {
		CacheArchive archive = CacheArchive.forFileIndex(request.getDataType());
		System.out.println("Missing file " + request.getId() + " from archive " + archive.toString().toLowerCase());
		if (archive == CacheArchive.MAPS_STANDARD || archive == CacheArchive.MAPS_OSRS) {
			System.out.println("Region id: " + Client.regionId);
		}
	}
	...
	public void method560(int file, int cache) {
		...
		if (file >= fileStatus[cache].length || fileStatus[cache][file] == 0) {//here
			return;
		}
		if (anInt1332 == 0) {
			return;
		...
		//System.out.println("Requesting2 " + CacheArchive.forFileIndex(cache) + " file " + file);
	...
	public void requestFileData(int dataType, int fileID) {
		...
		//System.out.println("Requesting1 " + CacheArchive.forFileIndex(dataType) + " file " + fileID);
	...
	public void start(Archive streamLoader, Client client1) {
		...
		/*String as1[] = { "model_crc", "anim_crc", "midi_crc", "map_crc", "textures_crc" };
		...
		for (int index = 0; index < fileStatus.length; index++) {
			fileStatus[index] = new byte[90_000];
		}
		...
		mapIndices1 = new int[2][];
		mapIndices2 = new int[2][];
		mapIndices3 = new int[2][];
		mapIndices4 = new int[2][];

		abyte2 = streamLoader.get("map_index");
		ByteBuffer stream2 = new ByteBuffer(abyte2);
		int mapCount = stream2.readUnsignedShort();
		mapIndices1[0] = new int[mapCount];
		mapIndices2[0] = new int[mapCount];
		mapIndices3[0] = new int[mapCount];
		mapIndices4[0] = new int[mapCount];
		int[] dntUse = new int[] {5150, 5160};//new int[] {5181, 5182, 5183, 5184, 5180, 5179, 5175, 5176, 4014, 3997, 5314, 5315, 5172};
		for(int i2 = 0; i2 < mapCount; i2++)
		{
			mapIndices1[0][i2] = stream2.readUnsignedShort();
			mapIndices2[0][i2] = stream2.readUnsignedShort();
			mapIndices3[0][i2] = stream2.readUnsignedShort();
			for(int i : dntUse)
			{
				if(mapIndices2[0][i2] == i)
					mapIndices2[0][i2] = -1;
				if(mapIndices3[0][i2] == i)
					mapIndices3[0][i2] = -1;
			}
		}

		abyte2 = Archive.ReadFile(Signlink.getCacheDirectory() + "/osrs/map_index");
		stream2 = new ByteBuffer(abyte2);
		mapCount = stream2.readUnsignedShort();
		mapIndices1[1] = new int[mapCount];
		mapIndices2[1] = new int[mapCount];
		mapIndices3[1] = new int[mapCount];
		mapIndices4[1] = new int[mapCount];
		for(int i2 = 0; i2 < mapCount; i2++)
		{
			mapIndices1[1][i2] = stream2.readUnsignedShort();
			mapIndices2[1][i2] = stream2.readUnsignedShort();
			mapIndices3[1][i2] = stream2.readUnsignedShort();
			// Some weird bugs with this so going to have to define to region ids
//			int region = mapIndices1[1][i2];
//			int x = ((region >> 8) << 6) + 10_000;
//			int y = ((region & 0xFF) << 6);
//            int regionX = x >> 3;
//            int regionY = y >> 3;
//            int regionId = (regionX / 8 << 8) + regionY / 8;
//            mapIndices1[1][i2] = regionId;
		}
	...
	public void resetOsrsRegions() {
		osrsRegions.clear();
	}

	public void addOsrsRegion(int regionId) {
		osrsRegions.add(regionId);
	}
