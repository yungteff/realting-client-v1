...
public class RSInterface {
		...
		for (int index = 0; index < interfaceCache.length; index++) {
			if (interfaceCache[index] == null) {
				...
				if (index - startIndex >= 100) {
					System.out.println("Found empty interface section starting at " + startIndex + ", length " + (index - startIndex));
				}
				startIndex = 0;
			}
		}
	...
	public static void addButton(int id, int sid, String tooltip) {
		addButton(id, sid, tooltip, Client.cacheSprite[sid].myWidth, Client.cacheSprite[sid].myHeight);
	}
		...
		while (stream.position < stream.buffer.length) {
			...
			if (i1 > 0) {
				...
				for (int j1 = 0; j1 < i1; j1++) {
					...
					rsInterface.requiredValues[j1] = stream.readUnsignedShort();
				}
			}
			...
			if (rsInterface.type == 4) {
				rsInterface.message = stream.readString().replaceAll("RuneScape", Configuration.CLIENT_NAME);
				rsInterface.aString228 = stream.readString();
			}

			if (rsInterface.type == 1 || rsInterface.type == 3 || rsInterface.type == 4) {
				rsInterface.textColor = stream.readIntLittleEndian();
			}

			if (rsInterface.type == 3 || rsInterface.type == 4) {
				rsInterface.anInt219 = stream.readIntLittleEndian();
				rsInterface.anInt216 = stream.readIntLittleEndian();
				rsInterface.anInt239 = stream.readIntLittleEndian();
			}
	...
	public int[] getChildCoordinates(int childInterfaceId) {
		for (int index = 0; index < children.length; index++) {
			if (children[index] == childInterfaceId) {
				return new int[] {childX[index], childY[index]};
			}
		}
	...
	private Model getMediaModel(int i, int j) {
		...
		if (i == 1) {
			model = Model.fetchModel(j, DataType.STANDARD);
		}
		if (i == 2) {
