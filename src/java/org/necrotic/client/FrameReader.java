...
public final class FrameReader {
	...
	public static void initialise(int i) {
		animationlist = new FrameReader[20_000][0];
	}
	...
	public static FrameReader forId(int i) {
		try {
			...
			int file = Integer.parseInt(s.substring(0, s.length() - 4), 16);
			int frameId = Integer.parseInt(s.substring(s.length() - 4), 16);

			CacheArchive archive = CacheArchive.ANIMATIONS_STANDARD;
			if (frameId >= 10_000) {
				archive = CacheArchive.ANIMATIONS_OSRS;
				//System.out.println("osrs file id: " + file);
				frameId -= 10_000;
				file += 10_000;
			}

			if (animationlist[file].length == 0) {
				//System.out.println("Requesting animation file: " + file);
				int realFileId = archive == CacheArchive.ANIMATIONS_STANDARD ? file : file - 10_000;
				Client.instance.onDemandFetcher.requestFileData(archive.getFileIndex(), realFileId);
				return null;
			} else {
				return animationlist[file][frameId];
			}
		} catch (Exception e) {
			e.printStackTrace();
			return null;
